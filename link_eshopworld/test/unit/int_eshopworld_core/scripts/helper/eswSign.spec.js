...
describe('/link_eshopworld/cartridges/int_eshopworld_core/cartridge/scripts/jwt/sign.js', function () {
    var sign = proxyquire('../../../../../cartridges/int_eshopworld_core/cartridge/scripts/jwt/sign.js', {
    ...
    describe('signJWT', function () {
        ...
        const options = {
            algorithm: "HS256",  // Choose an algorithm (HS256, RS256, etc.)
            kid: "example-key-id",  // The key ID
            privateKeyOrSecret: "mysecretkey123"  // The secret or private key
        };
        // Unit test
        it('return sign JWT', function () {
            let signJWT = sign.signJWT(payload, options);
            expect(signJWT).to.be.not.null;
        });
    });
});
