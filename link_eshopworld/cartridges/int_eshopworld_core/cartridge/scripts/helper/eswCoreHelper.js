...
const getEswHelper = {
    ...
    getOrderDiscount: function (cart, localizeCountryObj) {
        try {
            ...
            let that = this;
            let orderLevelProratedDiscount = that.getOrderProratedDiscount(cart, !empty(localizeCountryObj) ? localizeCountryObj.applyRoundingModel : { applyRoundingModel: true });
     ...
     * @param {dw.order.Basket} cart - DW Basket object
     * @param {Object} localizeCountryObj - localize country object
     * @returns {number} - orderLevelProratedDiscount
     */
    getOrderProratedDiscount: function (cart, localizeCountryObj) {
        let applyRoundingModel = typeof localizeCountryObj !== 'undefined' && !empty(localizeCountryObj.applyRoundingModel) && localizeCountryObj.applyRoundingModel === true;
        ...
        while (allPriceAdjustmentIter.hasNext()) {
            ...
            if (eachPriceAdjustment.priceValue) {
                if (applyRoundingModel) {
                    discountValue = this.getMoneyObject((eachPriceAdjustment.priceValue), false, false, true, localizeCountryObj, true).value;
                } else {
