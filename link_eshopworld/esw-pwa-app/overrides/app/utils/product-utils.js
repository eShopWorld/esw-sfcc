...
export const getPriceData = (product, opts = {}) => {
   ...
   // Get the highest list price from the current variant's tieredPrices, or fallback to highestTieredPrice
    const listPrice = product?.type?.variant
    ? product.variants?.find((variant) => variant.productId === product.id)?.tieredPrices?.reduce(
          (maxPrice, tier) => Math.max(maxPrice, tier.price),
          0
      )
    : highestTieredPrice?.price;
