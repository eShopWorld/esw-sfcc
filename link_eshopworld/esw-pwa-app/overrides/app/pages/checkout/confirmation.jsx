...
import {getOrderNumberHelper, getCookie} from '../../esw/esw-helpers'
...
const CheckoutConfirmation = () => {
    ...
    const [eswOrderNumber, setEswOrderNumber] = useState(null)
    ...
    useEffect(() => {
        const storedCustomerSession = localStorage.getItem('userSessionId')
        const eswSessionId = getCookie('esw.sessionid')
        if (!storedCustomerSession || eswSessionId !== storedCustomerSession) {
            navigate('/login')
        }
    }, [navigate])
    // Esw Customization End */

    // Get ESW order number first
    useEffect(() => {
        const getEswOrderNumber = async () => {
            if (!orderNo) return
            ...
            try {
                const response = await getOrderNumberHelper(orderNo)
                const data = await response.json()

                if (data.orderNumber) {
                    setEswOrderNumber(data.orderNumber)
                } else {
                    setOrderNumberError(data.error || 'Failed to get ESW order number')
                }
            } catch (error) {
                console.error('Error getting ESW order number:', error)
                setOrderNumberError(error.message || 'Failed to get ESW order number')
            } finally {
    ...
    // Only proceed with useOrder after getOrderNumberHelper has completed
    const shouldFetchOrder = !orderNumberLoading && (eswOrderNumber || orderNumberError)

    // Use the ESW order number if available, otherwise fall back to the original orderNo
    const finalOrderNumber = eswOrderNumber || orderNo

    const {data: order} = useOrder(
        {
            parameters: {orderNo: finalOrderNumber}
        },
        {
            enabled: Boolean(shouldFetchOrder && finalOrderNumber && onClient)
        }
    )
        ...
        <Box background="gray.50">
            ...
            >
                <Stack spacing={4}>
                    ...
                    {customer.isGuest && (
                        <>
                            {order?.c_isCheckoutRegisterationEnabled ? (
                                <Box
                                    layerStyle="card"
                                    rounded={[0, 0, 'base']}
                                    px={[4, 4, 6]}
                                ...
                                >
                                    <Flex justify="center">
                                        <Stack spacing={4}>
                                            <Button
                                                as="a"
                                                href={order?.c_checkoutRegisterationRedirectUrl}
                                                target="_blank"
