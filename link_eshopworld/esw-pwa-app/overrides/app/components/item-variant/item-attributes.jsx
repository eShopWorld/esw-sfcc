...
const ItemAttributes = ({includeQuantity, currency, ...props}) => {
    // ESW customization
    const [eswGeneralPrice, setEswgeneralPrice] = useState('')
    useEffect(() => {
        // compute class once (or whenever dependencies change)
        const eswGeneralPrice = applySparkClassToDom('esw-general-price')
        if (eswGeneralPrice) {
            setEswgeneralPrice(eswGeneralPrice)
        }
    ...
    return (
        <Stack spacing={1.5} flex={1} {...props}>
            ...
            {includeQuantity && (
                <Text lineHeight={1} className="esw-item-quantity" color="gray.700" fontSize="sm">
                    <FormattedMessage
            ...
            {variant.priceAdjustments?.length > 0 && (
                <Flex alignItems="center">
                    <Text lineHeight={1} color="gray.700" fontSize="sm">
                        ...
                        {': '}
                        {/* ESW customization */}
                        <Text as="span" color="green.700" className={eswGeneralPrice}>
                            <FormattedNumber
                                style="currency"
