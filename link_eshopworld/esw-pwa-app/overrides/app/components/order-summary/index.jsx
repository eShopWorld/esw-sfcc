...
import {useProducts, useShopperBasketsMutation} from '@salesforce/commerce-sdk-react'
...
const OrderSummary = ({
    ...
    setCurrency(
        basket.currency && basket.currency === selectedCountrycCurrency
            ? basket.currency
            : selectedCountrycCurrency
    )
    // End ESW customization
    useEffect(() => {
        setIsNativeShippingEnabled(getEswConfigByKey('eswNativeShippingEnabled'))
        const message = getEswConfigByKey('eswNativeShippingEnabledMsg')
        if (message && message !== null) {
            setNativeShippingEnabledMessage(message)
        }
    }, [])
    // ESW Native Shipping Imports
    ...
    // ESW Shipping Customization
    const [shippingTotal, setShippingTotal] = useState(basket?.shippingTotal)
    const [selectedShippingMethod, setSelectedShippingMethod] = useState(null)
    const isTaxEnabled = getEswConfigByKey('eswEnableTaxInformation')
    const {site, locale} = useMultiSite()

    const handleShippingMethodChange = (e) => {
        const selectedMethod = basket.c_available_shipping_methods.find(
        ...
        <Stack data-testid="sf-order-summary" spacing={5}>
            ...
            >
                ...
                <Stack w="full">
                    ...
                    {(!locale.isSupportedByESW || !isTaxEnabled) && (
                    <Flex justify="space-between" aria-live="polite" aria-atomic="true">
                        <Text fontSize={fontSize}>
                ...
                {/* ESW Override shipping methods */}
                {basket?.c_available_shipping_methods &&
                    basket.c_available_shipping_methods.length > 1 && (
                        <Stack spacing={4} w="full">
                            ...
                            <Select
                                ...
                                value={basket.shipments[0].shippingMethod.id} // Control the selected option dynamically
                                bg="white"
                                size="lg"
                            >
                                {basket.c_available_shipping_methods.map((method) => (
                ...
                <Stack spacing={4} w="full">
                    ...
                    {/* ESW shipping Message Display */}
                    {isNativeShippingEnabled && (
                        <Stack>
                            <Alert status="info">{nativeShippingEnabledMessage}</Alert>
                        </Stack>
                    )}
                    {/* ESW shipping Message Display */}
                </Stack>
