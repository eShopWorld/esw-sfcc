/*
 * Copyright (c) 2023, Salesforce, Inc.
 * All rights reserved.
 * SPDX-License-Identifier: BSD-3-Clause
 * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/BSD-3-Clause
 */

/**
 * ESW instrctions:
 * Make sure to use your default country and all related values in this json
 * After removing country from the BM, make sure to redeploy (not push) the build from MRT
 */

// Provide the sites for your app. Each site includes site id, and its localization configuration.
// You can also provide aliases for your locale. They will be used in place of your locale id when generating paths across the app
module.exports = [
    {
        id: 'RefArch',
        l10n: {
            supportedCurrencies: ['USD'],
            defaultCurrency: 'USD',
            defaultLocale: 'en-US',
            supportedLocales: [
                {
                    id: 'en-US',
                    alias: 'us',
                    preferredCurrency: 'USD',
                    supportedLocales: ['en-US'],
                    isFixedPriceModel: true,
                    isSupportedByESW: true,
                    countryCode: 'US'
                }
            ]
        }
    }
]
