...
function convertPromotionMessage(promotionMessageString) {
    ...
    let PromotionMessageWithoutHtml = promotionMessageString.replace(/<[^>]*[>$]|&nbsp;|&zwnj;|&raquo;|&laquo;|&gt;/g, '');
    ...
    let selectedCountryAdjustment = eswHelper.strToJson(session.privacy.countryAdjustment || null);
    let enableRounding = eswHelper.isEswRoundingsEnabled();
    let disableRounding = promotionMessageString.indexOf('data-disable-rounding') !== -1;
    let disableAdjustment = promotionMessageString.indexOf('data-disable-adjustments') !== -1;
    ...
    let eswPrice = (!empty(discountPriceFromHtml)) ? Number(discountPriceFromHtml) : 0;
    ...
    eswPrice = Number((eswPrice * selectedFxRate.rate).toFixed(3));
    if (!disableRounding && enableRounding) {
        eswPrice = eswHelper.applyRoundingModel(eswPrice);
    }
    let promotionMessageFinal = PromotionMessageWithoutHtml
